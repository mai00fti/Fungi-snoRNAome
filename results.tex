...
snoRNA complement by \Sebastian{XXX$\%$} and immensely increased its resolution. 
...
Box C (RTGATGA) and D (CTGA) \JH{(We are talking about RNAs, shouldn't we $s/T/U/g;$?)} match the consensus sequence motifs
almost perfectly. Box C shows a purine (R) in 92\% of all cases. 
The 5' GA di-nucleotide is conserved to 100\%. 
The 5' nucleotide \JH{C or which one?} of box D
...
In box C', merely the 5' TG \JH{(UG)} di-nucleotide and, to a lesser extent, the
...
3$^{rd}$ position while the trailing \JH{(trailing $\neq$ 3$^{rd}$ position???)} adenine is more variable. The
...
In contrast, \haca s are required to develop a significant and specific secondary structure in order to function appropriately. Only 15\% (395 out of 2269) of all snoRNAs were not predicted to fold into a stem-loop structure for both hairpins. 
...
or \sli: 0,86) \JH{(\% ??)}. The impact on \cd s is more moderate (0.26 on
