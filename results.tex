\NEW{Up to now, there is no generally accepted nomenclature of snoRNA
  families across different fungal species and quiet a few \sno\ genes
  of distinct organisms are named differently although these genes
  belong to the same \sno\ family. Here, we present the first complete
  and reliable mapping of \sno\ names, that is entirely based on
  sequence and functional homologies across the kingdom of fungi.}  In
the following we will use established gene names to designate snoRNA
families where possible. In cases where homologs have different names
in different species we use the preferred order \emph{S.\ cerevisiae},
\emph{N.\ crassa}, \emph{A.\ fumigatus}, \emph{C.\ albicans}, and
\emph{S.\ pombe}. To simplify cross-referencing with machine readable
data we also list the \snostrip\ family designations in parentheses.
A complete dictionary of nomenclature correspondences can be found in
Supplementary Table S3.  Similarly, we pragmatically identify target
positions with their position in the multiple sequence alignments of
the target RNAs. Coordinates for reference sequences from selected
organisms are given in parentheses.  Single sequence target RNAs and
target RNA alignments are provided in Supplementary Table S4.
...
more than \NEW{200\% compared to our previously available \sno\ set
  \cite{Bartschat:2014} and over 450\% compared to \rfam{}-annotated
  \sno s}. This massive amount of \sno\ data substantially increases both the
phylogenetic depth and the resolution of the snoRNA annotation.
...
\NEW{Due to our comprehensive \sno\ dataset, it is now feasible to
  thoroughly examine the pyhlogenetic fundmanent of fungal \sno s.}  A
...
\NEW{Despite some single-species studies, a thorough functional
  analysis of the entire fungal \sno ome has not been done before but
  is essential to sort and clarify the \sno\ landscape (identification of
  functional homologous sequences) and to further investigate
  functional rearrangements and other peculiarities.}
...
guides \NEW{meaning that these families share exactly one conserved
  target region} (of these, 28 families share a functional D' target
...
shifted. 
\NEW{In the following, we will present an in-depth description of 
  highly complex rearrangments including target translocations and
  duplications between several \sno\ families that have been automatically uncovered by \snostrip.}  

Each of these two '\sno\ clans' comprise two,
...
To verify our \sno\ data and to add further reliability, we compared
...
confidence sequences, typically with direct experimental support. \PFS{The
  7852 \snostrip -detected \sno s encompass the overwhelming majority of
...
compared to the consensus sequence. \PFS{This prevents \snostrip\ from
  including these sequences} into our data set even though their detection
by means of \blast\ or \infernal\ is straightforward.
}
...
experimentally verified \PFS{\sno\ that served as the starting point for
  ...
  64 of the 75 \sno{}s published in \cite{Liu:2009} have not found their way into \rfam.}}
...
sequences of the \rfam\ models RF01209 and RF01514, \PFS{which were derived
  ...
  pairs most likely exhibit the highest similarity.} A table listing all 18 \sno\
families and their respective \rfam\ models can be seen in section S10
of the supplement alongside figures showing the \texttt{CMcompare}\
score distribution. 
}

\paragraph{\textbf{Ribosome footprinting}}
\NEW{Ribo-seq or ribosome profiling combines ribosome footprinting
  with deep sequencing \cite{Ingolia:2009,Ingolia:2012} to study
  translation \textit{in vivo}. Not all the detected events, however,
  correspond to ribosome-protected mRNA fragments and hence to
  translated regions.  As a byproduct, other non-ribosomal RNA-protein
  complexes are also detected, including transfer RNAs, spliceosomal
  RNAs, and snoRNAs \cite{Ingolia:2014, Ji:2016}. \PFS{We therefore
    used publicly available Ribo-seq data from four different fungi
    (\Sce\ \cite{Ingolia:2009}, \Spo\ \cite{Duncan:2014}, \Calb\
    \cite{Muzzey:2014}, and \Aca\ \cite{Gilmore:2015}) to support the
    identification of novel \sno{}s.}  \PFS{Importantly, all \sno s in
    \sce\ were based on independent experimental studies, that serve
    as a positive control for the use of Ribo-seq data.} In addition
  to the raw read data, we also evaluated the localization of these
  reads when overlapping with our \snostrip\ annotation. Non-ribosomal
  protein-associated RNAs show a highly localized read pattern in
  contrast to mRNAs where one would expect a rather uniformly
  distributed pattern with a clearly visible 3nt periodicity.  We
  therefore used the Percentage of Maximum Entropy (PME) to quantify
  the uniformity of read distribution across the \sno -annotated
  regions, employing the \texttt{Rfoot} tool \cite{Ji:2016}. The
  complete results are shown in section S11 in the supplement.

  Considering the \sce\ data, all annotated \sno s show overlapping
  Ribo-seq reads in at least 4 of the 14 libraries provided by
  \cite{Ingolia:2009}, with evidence from every dataset supporting 52
  \sno s. 65 \sno s are classified as 'non-coding' based on their
  PME-scores. The complete results can be seen in section S11 in the
  supplement.  In case of \spo, 61 of 62 \sno\ are present in at least
  one of the three short read archives, with 46 \sno s supported in in
  all available data sets. Based on their PME-score, 49 annotated
  genes are classified as 'non-coding'. For \aca\ and \calb\, all
  \snostrip -annotated sequences have Ribo-seq support from at least
  one experiment, while 60 (of 62) and 45 (of 67) are present in all
  datasets, respectively. In \aca, all \sno\ genes classified as
  'non-coding', in \calb, on the other hand, two sequences lack
  support from the PME scoring. Almost all of the \sno s without PME
  score support lack sufficient read coverage for the \texttt{Rfoot}
  analysis. Even though, the available Ribo-seq data strongly supports
  our \snostrip -annotated datasets and hence, adds another convincing
  layer of reliability to our fungal \sno ome.  }
