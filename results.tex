...
There is at present no generally accepted nomenclature of snoRNA families
across different fungal species. In the following we will use established
gene names to designate snoRNA families where possible. In cases where
homologs have different names in different species we use the preferred
order \emph{S.\ cerevisiae}, \emph{N.\ crassa}, \emph{A.\ fumigatus},
\emph{C.\ albicans}, and \emph{S.\ pombe}. To simplify cross-referencing
with machine readable data we also list the \snostrip\ family designations
in parentheses.  A complete dictionary of nomenclature correspondences can
be found in Supplementary Table S3.  Similarly, we pragmatically identify
target positions with their position in the multiple sequence alignments of
the target RNAs. Coordinates for reference sequences from selected
organisms are given in parentheses.  Single sequence target RNAs and target
RNA alignments are provided in Supplementary Table S4.
...
\sno\ family is published separately due to its special function,
various splice variants and characteristics \cite{Canzler:2017}. All
...
incorrect annotations.  In total we found 5593 \cd\ and 2255 \haca\
sequences, expanding the collection of annotated fungal snoRNAs by
more than \SB{200\% compared to our previous \sno\ set
  \cite{Bartschat:2014} and over 450\% compared to \rfam -annotated
  \sno s}. The data substantially increase both the phylogenetic depth
and the resolution of the snoRNA annotation.
...
guides \SB{meaning that these families share one conserved target region only} (of these, 28 families share a functional D' target and 12 a
conserved D target). Another 14 \cd\ families are \textit{predominantly}
single guides, i.e., these families share exactly one strongly
conserved target binding region (three families share a conserved D target
while 11 families share a functional D' target), while the other target
region is only found to be functional in a restricted subset of taxa. Only
eight families harbor two functional target binding regions that are
conserved throughout all the lineages in which the \sno\ families are
present. The remaining six families stay orphan, i.e., no potential
interaction has been published so far.
...
\paragraph{\textbf{Comparison to \rfam}}
The \rfam\ database \cite{Nawrocki:2015} is by far the most comprehensive source for
high-quality non-coding RNAs. Since our extensive collection of \sno\
families should be submitted to \rfam, we cross-checked our results
with the previous \rfam -annotated \sno s. The current \rfam\ (version
12.3) covers 755 snoRNA models. 116 of them contain at least one
fungal \sno\ of our 147 investigated organisms. In total, these 116
models contain 1621 \sno\ sequences found in our set of organisms, 457
of which are in seed alignments indicating their high confidence. When
comparing with the set of 7852 \snostrip -detected \sno s, the vast
majority of \rfam -annotated sequences is also identified with our
annotation pipeline (1586 \sno\ genes). 441 sequences are located in
seed alignments. In total, \snostrip\ was not able to annotate merely
69 \sno\ sequences, due to a rather technical obstacle. The
overwhelming majority of these missing genes contains box motifs with
two or more mutations compared to the consensus sequence. This
prohibited our pipeline to include the \sno s in our dataset although
their sheer detection by means of \blast\ or \infernal\ was straight
forward.

In principal, in 32 \snostrip -annotated \sno\ families we were not
able to identify any of its \sno\ sequences within \rfam\ even though
each family contains at least one experimentally verified starting
sequence. This mainly concerns \ncr\ sequences, were 64 out of 75
originally published \sno\ sequences are not contained in \rfam.

On 18 occasions, \snostrip\ was able to automatically combine \sno\
sequences of two different \rfam\ models into one \sno\ family. These
models contain a distinct set of organism but share the same target
guiding functionality. For example, family CD\_12 (\sce: snR76)
combines sequences of the \rfam\ models RF01209 and RF01514, that
consists of Saccharomycetales and Pezizomycotina species,
respectively. Both models were scored with \texttt{CMcompare} against
all \rfam\ \sno\ models to validate the conflation made by \snostrip\
\cite{Eggenhofer:2013}. For both models, the assigned pairwise score
to one another was ranked at the top spot suggesting a true
sequence/structure-based similarity between these two models. A table
listing all 18 \sno\ families can be seen in section S10 of the
supplement.

\TODO{ Highlight the major benefit for \rfam\ since we provide a huge
  amount of new \sno\ sequences, but also several families, that are
  not incorporated yet. --> Discussion?}  }


\paragraph{\textbf{Ribosome footprinting}}
\SB{ In general, Ribo-seq or ribosome profiling combines ribosome
  footprinting with deep sequencing \cite{Ingolia:2009,
    Ingolia:2012}. This technique is widely used to study translation
  \textit{in vivo}, although not all detected events correspond to
  ribosome-protected mRNA fragments and hence translated regions.  As
  a byproduct, other non-ribosomal RNA-protein complexes are also
  present including, for example, transfer RNAs, spliceosomal RNAs,
  and snoRNAs \cite{Ingolia:2014, Ji:2016}.

  Here, we used available Ribo-seq reads in four different fungal
  organisms to verify our set of predicted \sno s. This includes \Sce\
  \cite{Ingolia:2009}, \Spo\ \cite{Duncan:2014}, \Calb\
  \cite{Muzzey:2014}, and \Aca\ \cite{Gilmore:2015}. Since all of our
  \sno s in \sce\ were already annotated and experimentally validated
  before, the comparison with Ribo-seq data serves as a litmus test.

  Besides raw sequence reads, we also looked at the localization of
  these reads when overlapping with our \snostrip\
  annotation. Non-ribosomal protein-associated RNAs show a highly
  localized read pattern in contrast to mRNAs where one would expect a
  rather uniformly distributed pattern with a clearly visible 3nt
  periodicity. To take the circumstances into consideration, we
  calculated the Percentage of Maximum Entropy (PME) to measure the
  uniformity of read distribution in our \sno -annotated regions with
  the help of the \texttt{Rfoot} tool \cite{Ji:2016}. 
  %A PME-score of 0
  %would describe a read distribution where all reads are located at the
  %exact same position whereas a score of 1 equals an absolute uniform
  %distribution.

  When looking at \sce, all annotated \sno s show overlapping Ribo-seq
  reads in at least 4 of the 14 libraries provided by
  \cite{Ingolia:2009}, while we find evidence in all datasets for 52
  \sno s. 65 \sno s are classified as 'non-coding' based on their
  PME-score. The complete results can be seen in section S11 in the
  supplement.  In case of \spo, 61 of 62 \sno\ are present in at least
  one of the three short read archives, whereas we find overlapping
  reads for 46 \sno s in all datasets. Based on their PME-score 49
  annotated genes are found to be 'non-coding'. For \aca\ and \calb\,
  all \snostrip -annotated sequences have Ribo-seq overlapping reads
  in at least one experiment, while 60 (of 62) and 45 (of 67) are
  present in all datasets, repsectively. In \aca are all \sno\ genes
  attested to be 'non-coding' by \texttt{Rfoot} whereas in \calb two
  sequences lack this sort of confirmation.

  Please note that the vast majority of \sno s that are not classified
  as 'non-coding' through their PME-score, have too few reads such that
  these genes are not considered for the \texttt{Rfoot} analysis.

  In general, the comparison with available Ribo-seq data clearly
  substantiates the \snostrip -annotated \sno ome and adds an
  additional layer of reliablity to our dataset.
