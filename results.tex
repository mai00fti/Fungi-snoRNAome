\TODO{I understand the previous paragraph in Fungi\_snoRNAome.tex is
  outdated and should be replaced by the following:} 
\newcommand{\PFS}[1]{\begingroup\color{blue}#1\endgroup}
\PFS{There is at present no generally accepted nomenclature of snoRNA
  ...
  perference order \emph{S.\ cerevisiae}, \emph{N.\ crassa}, \emph{A.\
    fumigatus}, \emph{C.\ albicans}, and \emph{S.\ pombe}. To simplify
  cross-referncing with machine readable data we also list the \snostrip\
  family designations in parentheses.  A complete dictionary of
  nomenclature correspondences can be found in Supplementary Table S3.
  Similarly, we pragmatically identify target positions with their position
  in the multiple sequence alignments of the target RNAs \TODO{Are the
    alignments included? refer to them!}  Coordinates for reference
  sequences from selected organisms are given in parentheses.}
...
is considered separately due to its special function and characteristics
and published elsewhere \cite{Canzler:2017}. All \snostrip\ candidates were
carefully cross-checked in all species to reduce the number of false
negatives and to exclude potentially incorrect annotations.  In total we
found 5595 \cd\ and 2331 \haca\ sequences, expanding the collection of
annotated fungal snoRNAs by more than 120\%. The data substantially
increase both the phylogenetic scope and the resolution of the snoRNA
annotation.
...
consistent with the published rules \TODO{insert refs here} for canonical
\sno\ box motifs known from both yeast and animals: 
...
families \PFS{with higher resolution} is included as Supplementary Figure
S6. 
...
moderate (0.26 \TODO{is the the average number, percentage, or do you mean
...
families a conserved D box associated binding site. \TODO{this is
  confusing: how does this add up?}  In addition, 14 \cd\ families are
...
Family snR3 is known \TODO{include reference} to guide three targets in
...
Neurospora publication \TODO{replace by reference}, however, HP1 is
...
\TODO{Add units kcal/mol consistently. Whenever refererring to 
known, experimentally validated etc include refernce!}
...
18S-479 (18S-436; D' target) \TODO{include ref}, is detected in all
...
Similar to the box C/D \sno\ class, several \haca s have a
...
Maybe a \sno\ of clan CD\_5 guiding methylation at 25S-3615 was already
...
\TODO{Maybe move this to the discussion/conclusion section?}
...
and experimentally validated \TODO{ref} to guide methylation at position
