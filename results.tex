\NEW{Up to now, there is no generally accepted nomenclature of snoRNA
  ...
  and reliable mapping of \sno\ names across the kingdom of fungi, that is entirely based on
  sequence and functional homologies.}  In
...
\NEW{Due to our comprehensive \sno\ dataset, it is now feasible to
  thoroughly examine the phylogenetic fundament of fungal \sno s.}
...
guides \NEW{meaning that these families share exactly one conserved
  target region} (28 families share a functional D' target
...
Seven of 15 orphan \haca s are found to be merely conserved on genus
...
other predicted positions have not been shown to be chemically
modified so far.
...
archives of four different Fungi. This should allow us to point out
that our \snostrip\ annotations are in line with existing \rfam\
...
that are not included in our dataset exhibit box motifs with two or more mutations
compared to the consensus sequence. This prevents \snostrip\ from
  including these sequences into our dataset even though their detection
...
experimentally verified \sno\ that served as the starting point for
  ...
  64 of the 75 \sno s proclaimed in \cite{Liu:2009} have not found their way into \rfam.}
...
\NEW{Ribo-seq or ribosome profiling combines ribosome footprinting
  ...
  percentage of maximum mntropy (PME) scores calculated by the \texttt{Rfoot} tool \cite{Ji:2016}. The
  complete results can be seen in Supplementary Section S11. In case
  of \spo, 61 of 62 \sno\ are present in at least one of the three
  short read archives, with 46 \sno s supported in all available
  datasets. Based on their PME score, 49 annotated genes are
  classified as 'non-coding'. For \aca\ and \calb , all \snostrip
  -annotated sequences have Ribo-seq support from at least one
  experiment, while 60 (of 62) and 45 (of 67) are present in all
  datasets, respectively. In \aca, all \sno\ genes are classified as
  'non-coding', in \calb, on the other hand, two sequences lack
  support from the PME scoring. Almost all of the \sno s without
  PME score support lack sufficient read coverage for the
  \texttt{Rfoot} analysis.  Even though, the available Ribo-seq data
