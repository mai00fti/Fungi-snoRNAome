\PFS{So far,} there is no generally accepted nomenclature of snoRNA
...
\paragraph{\textbf{Box motifs}} Sequence motifs were extracted from all
...
\PFS{lengths of \cd{}s,} 90\% of the novel \snostrip-annotated \sno s are
...
In general, \sno -specific characteristics \PFS{such as} box motifs,
lengths, and secondary structures are \PFS{very similar} between Fungi and
Metazoa \cite{Kehr:2014}.

\subsection{Phylogenetic distribution of fungi snoRNAs}

\PFS{The comprehensive \sno\ dataset reported here makes it feasible} to
...
organism. Exceptions to this rule are \PFS{the} snoRNA 'clans' CD\_5 and
...
Almost half of the \cd\ families are traceable down to the root of fungi
...
\PFS{In addition to} lineage-specific families \PFS{there are families that
...
\PFS{We remark} that not a single \haca\ is found in \Ptt\ (marked with an
asterisk in the Supplementary Figure S7.2). \PFS{This observation is in}
...
(HACA\_46)). \PFS{We} note that family HACA\_2, despite its early origin,
...
other \PFS{hand}, is exclusively found in Saccharomycetaceae.
...
are also predicted in \afu. Both remaining families (marked \PFS{by} '*' in
...
\PFS{homology to other known} \sno s comprised 16 distinct targets
...
Seven of 15 orphan \haca s are found to be conserved \PFS{only} on genus or
...
\PFS{Several} \cd\ families harbor a highly conserved target either at
their D or D' position. \PFS{It may be the case, however, that many of}
...
and lineage-specific D targets \PFS{are shown} in Figure
...
Family snR87 (CD\_10), for example, with its experimentally verified target
...
Similar to the box C/D \sno\ class, several \haca s have a functional and
...
these functions might already be annotated, \PFS{in particular for} \sno\
