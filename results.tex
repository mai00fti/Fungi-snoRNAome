...
Each of these two '\sno\ clans' comprise two, three, or even more \sno\
...
large subunit ribosomal RNAs. Hence, they are \PFS{most likely} ancient,
which suggests \PFS{that} both the methylations and \PFS{their} guiding
snoRNAs \PFS{also existed} at the root of fungi.  However, through
...
putative \PFS{ancestral} state \PFS{probably consisted of} two individual
\sno\ sequences guiding the three ancient methylations. Deletion and
innovation events of target interactions \PFS{(inferred assuming
...
At the root of Ascomycota, a polycistronic \sno\ transcript \PFS{harboured}
...
several possibilities how \PFS{this} \sno\ cluster evolved after the
...
(25S-1866). \PFS{In this scenario, the original arrangement} is conserved
\PFS{only} in \Yli. \PFS{An alternative is outline by the} green stars in
...
\begin{table}
  ...
  \end{center}
\end{table}
An outstanding example \PFS{is the} \cd\ family snR40 (CD\_43), \PFS{which
...
the latter one, in the \PFS{overwhelming} majority of cases even with a
better binding energy. \PFS{However,} a genuine modification \PFS{of this
  site has not been reported} in \sce, \ncr, or human. \PFS{Albeit very
  compelling, the prediction thus remains a hypothesis.}

An even more impressive example is the D' ASE of the snR70 (CD\_61)
family.  \JH{No less (???)} than five potential targets are predicted with an ICI
