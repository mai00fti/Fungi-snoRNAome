...
more than \NEW{200\% compared to the previously available \sno\ set
...
guides \NEW{meaning that these families share one conserved target region
  only} (of these, 28 families share a functional D' target and 12 a
conserved D target). Another 14 \cd\ families are \textit{predominantly}
single guides, i.e., these families share exactly one strongly conserved
target binding region (three families share a conserved D target while 11
families share a functional D' target), while the other target region is
only found to be functional in a restricted subset of taxa. Only eight
families harbor two functional target binding regions that are conserved
throughout all the lineages in which the \sno\ families are present. The
remaining six families stay orphan, i.e., no potential interaction has been
published so far.
...
source \PFS{of well-curated data on} non-coding RNAs. The extensive
collection of \sno\ families \PFS{reported here is intended to be
...
covers 755 snoRNA models. 116 \PFS{families} contain at least one fungal
...
confidence sequences, typically with direct experimental support. \PFS{The
  7852 \snostrip -detected \sno s emcompass the overwhelming majority of
  \rfam -annotated sequences (1586 of 1621 \sno\ genes),} and 441 sequences
are located in seed alignments. \PFS{Most of the 69 \rfam\ \sno\ sequence not
  recognized by \snostrip\ have} box motifs with two or more mutations
compared to the consensus sequence. \PFS{This prevents \snostrip\ from
  including these sequences} into to data set even though their detection
by means of \blast\ or \infernal\ was straightforward.

\PFS{Conversely, 32 of the \snostrip -annotated \sno\ families had no
  representative in \rfam, even though they harbour} at least one
experimentally verified \PFS{\sno\ that served as the starting point for
  homology search in this study. Most of these are attested for \ncr, where
  64 of the 75 \sno{}s published in \cite{Liu:2009} \TODO{** check
    reference **} have not found their way into \rfam.}

In 18 cases \snostrip\ automatically combined \sno\ sequences of two
different \rfam\ models to a common \sno\ family. \PFS{A detailed
  inspection showed that merged \rfam\ models were constructed from
  disjoint sets of organism and indeed share the same target guiding
  functionality.} For example, family CD\_12 (\sce: snR76) combines
sequences of the \rfam\ models RF01209 and RF01514, \PFS{which were derived
  from} Saccharomycetales and Pezizomycotina species, respectively. \PFS{A
  pairwise comparison of the 18 families against all other \rfam\ models
  with \texttt{CMcompare} \cite{Eggenhofer:2013} showed that the merged
  pairs always exhibit the highest similarity.}  \TODO{can we show that the
  similarity is much larger than the background distribution of other
  pairs? Maybe a supplemental figure?}  A table listing all 18 \sno\
families can be seen in section S10 of the supplement.

\TODO{ Highlight the major benefit for \rfam\ since we provide a huge
  amount of new \sno\ sequences, but also several families, that are not
  incorporated yet. --> Discussion?}  
}
