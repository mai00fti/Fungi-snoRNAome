...
\NEW{Despite some single-species studies, a thorough functional
  ...
  functional re-arrangements and other peculiarities.}
...
\NEW{Ribo-seq or ribosome profiling combines ribosome footprinting with
  deep sequencing \cite{Ingolia:2009,Ingolia:2012} to study translation
  \textit{in vivo}. Not all the detected events, however, correspond to
  ...
  including transfer RNAs, spliceosomal RNAs, and snoRNAs
  \cite{Ingolia:2014, Ji:2016}. \PFS{We therefore used publicly available
    Ribo-seq data from four different fungi (\Sce\ \cite{Ingolia:2009},
    \Spo\ \cite{Duncan:2014}, \Calb\ \cite{Muzzey:2014}, and \Aca\
    \cite{Gilmore:2015}) to support the identification of novel \sno{}s.}
  \PFS{Importantly, all \sno s in \sce\ were based on independent
    experimental studies, that serve as a positive control for the use of
    Ribo-seq data.} In addition to the raw read data, we also evaluated the
  localization of these reads when overlapping with our \snostrip\
  annotation. Non-ribosomal protein-associated RNAs show a highly localized
  read pattern in contrast to mRNAs where one would expect a rather
  uniformly distributed pattern with a clearly visible 3nt periodicity.  We
  therefore used the Percentage of Maximum Entropy (PME) to quantify the
  uniformity of read distribution across the \sno -annotated regions,
  employing the \texttt{Rfoot} tool \cite{Ji:2016}.
  ...
  supplement.  In case of \spo, 61 of 62 \sno\ are present in at least one
  of the three short read archives, with 46 \sno s supported in in all
  available data sets. Based on their PME-score, 49 annotated genes are
  classified as 'non-coding'. For \aca\ and \calb\, all \snostrip
  -annotated sequences have Ribo-seq support from at least one experiment,
  while 60 (of 62) and 45 (of 67) are present in all datasets,
  respectively. In \aca, all \sno\ genes classified as 'non-coding', in
  \calb, on the other hand, two sequences lack support from the PME scoring. Almost all of the
  \sno s without PME score support lack sufficient read coverage
  for the \texttt{Rfoot} analysis. Even though, the available Ribo-seq data
  strongly supports our \snostrip -annotated datasets and hence, adds
  another convincing layer of reliability to our fungal \sno ome. 
}
