...
\usepackage{wrapfig,floatfig,epsfig, a4wide}
...
\newcommand{\Jana}[1]{\begingroup\color{purple}#1\endgroup}
%\newcommand{\KR}[1]{\begingroup\color{magenta}#1\endgroup}
\newcommand{\Sebastian}[1]{\begingroup\color{turquoise}#1\endgroup}
...
\author[UFZ]{Jana Hertel\corref{cor1}}
...
Localized in the nucleolus of the cell they associate with at least four proteins in a small nucleolar ribonucleoprotein (snoRNP) complex.
The target RNA molecule is hold in the correct position by base pairing to short unpaired region(s) within the snoRNA. 
These regions are referred to as antisense elements (ASE). 
At a specific position, the target is then modified.
Ribosomal RNAs (rRNAs) and small nuclear RNAs (snRNAs) constitute the main class of targets.
However, several snoRNAs have been shown to target residues in other (small) RNAs \cite{•}.
There are two distinct classes of snoRNAs: box C/D and box H/ACA snoRNAs. 
They are distinguished by their secondary structure, sequence features and the modification they guide. 
Box C/D snoRNAs form a stem-loop structure with a rather long loop which is stabilized by the associated proteins.
They guide the 3'OH-methylation of nucleotides.
Box H/ACA snoRNAs, on the other hand, are longer, fold into a thermodynamically more stable double stem-loop structure. 
These genes guide pseudouridylation of uracil residues in the target RNA.
In addition, there are chimeric snoRNAs that share features of both classes, are much longer and/or are described to have different functions. 
Like other small ncRNAs, snoRNAs are functional due to their secondary structure \emph{and} characteristic (short) regions in their primary structure. 
In consequence, selective pressure acts on these short sequence motifs (boxes and ASEs) and the preservation of the stem-loop(s) during evolution. 
This led to a low conservation of the \emph{overall} sequence, an increased number of compensatory mutations in structured regions and high sequence conservation at the protein and target RNA binding sites.
The annotation and classification of snoRNAs according to sequence homology only, e.g. by using \texttt{NCBI-blast}\cite{•} is therefore not feasible. 
Our recently  introduced computational annotation pipeline \snostrip\cite{Bartschat:2014} is specifically designed to take care of all specific characteristics of snoRNAs.
...
In this study we analysed a large set of fungal species with genomes that are available in decent quality for their snoRNA abundance. 
We started with experimentally verified snoRNAs in five fungi.
Further, we studied the evolutionary conservation of those snoRNAs and the co-evolution of snoRNAs and their targets. 
As a result we provide a comprehensive set of fungal snoRNAs, their detailed description with respect to genomic location, box motifs and position, potential/confirmed target information (including target switches), family assignment and evolutionary history. 
...
Genome sequences from 147 fungal species were downloaded from various sources, e.g. NCBI\cite{•}, ENSEMBL\cite{•}, USCS\cite{•}, etc. \Sebastian{(others? references?)} 
An \ncbi-based taxonomic tree displaying the relationship, genome source, and genome version of all fungal organisms in the evolutionary survey is shown in the supplement, see Figures S\ref{fig:complete_taxonomic_tree_upper_half} and S\ref{fig:complete_taxonomic_tree_lower_half}. 
A simplified taxonomic tree that shows the important fungal lineages is provided in Figure \ref{fig:simplified_tree}. 
...
For 63 out of the 147 species, snoRNA sequences were readily available at the \snostrip\ webservere\footnote{\url{http://snostrip.bioinf.uni-leipzig.de/}}. 
...
\ref{tab:original_snornas} \Sebastian{A complete mapping of the species-specific names as they were used in the respective publication and the internal \snostrip\ names is shown in supplementary Tables \ref{tab:haca_snoRNA_mapping} and \ref{tab:cd_snoRNA_mapping}. }. Note that annotated \afu\ \cd\  AM921943 is treated as a \haca.  It shows two separated,
...
All but budding yeast \sno s have been taken from the corresponding publication. Budding yeast sequences have been downloaded from the
\texttt{UMass}-database\footnote{http://people.biochem.umass.edu/fournierlab/snornadb/main.php}.
In total, this starting set of snoRNAs comprises more than 3500 snoRNA sequences assigned to 123 snoRNA families in the 63 species. \Sebastian{(Wrong(!) Table: Table 4.1: 68+50+1=119 != 120??) This is the final result of snoRNA families..., The 123 families are in the starting set...} 
...
%If this was not feasible due to non-canonical box motifs at the supposed positions, either the box annotation was deleted (prime boxes) or the whole \sno\ candidate was rejected. 
% since disrupted box motifs imply non-functional mature \sno s.
