...
However, several snoRNAs have been shown to target residues in \JH{other (small) RNAs \cite{•}}.
...
In addition, there are chimeric snoRNAs that share features of both classes, are much longer and/or are described to have different functions \JH{REFS!}. 
...
Genome sequences from 147 fungal species were downloaded from NCBI\cite{•}, ENSEMBL\cite{•}, USCS\cite{•}, \JH{etc. (explicitly name all that you used!)} 
An \ncbi-based taxonomic tree displaying the relationship, genome source, and version for all fungal organisms in the evolutionary survey is shown in the supplement, see Figures S\ref{fig:complete_taxonomic_tree_upper_half} and S\ref{fig:complete_taxonomic_tree_lower_half}. 
...
All other \sno s have been taken from the corresponding publication. In total, our starting set of snoRNAs comprises more than 3500 \JH{Provide the precise number!} snoRNA sequences assigned to 123 snoRNA families in the 63 species. \Sebastian{(Wrong(!) Table: Table 4.1: 68+50+1=119 != 120??) This is the final result of snoRNA families..., The 123 families are in the starting set...} \JH{??? watn nu?}
